...
var a = (o2) => "vertical" === o2 || "horizontal" === o2 || "cross" === o2 || "esriSFSCross" === o2 || "esriSFSVertical" === o2 || "esriSFSHorizontal" === o2;
function r(o2, r2, e2) {
  const i = c(Math.ceil(e2)), n = a(r2) ? 8 * i : 16 * i, s = 2 * i;
  ...
  const h = l.getImageData(0, 0, o2.width, o2.height), c2 = new Uint8Array(h.data);
  let m;
  for (let t = 0; t < c2.length; t += 4)
    m = c2[t + 3] / 255, c2[t] = c2[t] * m, c2[t + 1] = c2[t + 1] * m, c2[t + 2] = c2[t + 2] * m;
  return [c2, o2.width, o2.height];
}
function e(t, a2) {
  ...
  for (const o2 of t)
    l += o2;
  const h = Math.round(l * n), c2 = new Float32Array(h * s), m = 0.5 * n;
  ...
  for (const o2 of t) {
    for (S = F, F += o2 * n; g <= F; ) {
      ...
      for (; o3 < s; ) {
        ...
        c2[t2] = T ? r2 ? Math.max(Math.max(S + m - g, a3), Math.max(g - F + m, a3)) : a3 : i ? Math.min((g - S) * (g - S) + a3, (g - F) * (g - F) + a3) : e2 ? Math.min(Math.max(g - S, a3), Math.max(F - g, a3)) : Math.min(Math.max(g - S + m, a3), Math.max(F + m - g, a3)), o3++;
      }
      g++;
    }
    T = !T;
  }
  const d = c2.length, f = new Uint8Array(4 * d);
  for (let M = 0; M < d; ++M) {
    const t2 = (i ? Math.sqrt(c2[M]) : c2[M]) / n;
    o(t2, f, 4 * M);
  }
  return [f, h, s];
}
...
//# sourceMappingURL=chunk-FKUVQAN6.js.map
