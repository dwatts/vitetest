...
function n2(n3, l, r, u) {
  if (r.level === u.level)
    return l;
  const i = n3.tileInfo.size, a2 = n3.getTileResolution(r.level), c = n3.getTileResolution(u.level);
  let g = n3.getLODInfoAt(u.level);
  const h = g.getXForColumn(u.col), d = g.getYForRow(u.row);
  g = n3.getLODInfoAt(r.level);
  const f = g.getXForColumn(r.col), s = g.getYForRow(r.row), m = e2(l) / i[0], v = t2(l) / i[1], w = Math.round(m * ((h - f) / a2)), I = Math.round(v * (-(d - s) / a2)), M = Math.round(m * i[0] * (c / a2)), F = Math.round(v * i[1] * (c / a2)), L = o(i);
...
//# sourceMappingURL=chunk-SA34YTVI.js.map
